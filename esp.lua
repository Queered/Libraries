...
local destory_render_object = debug.getupvalue(debug.getupvalue(Drawing.new, 7).__index, 3);
local rotate_vector = function(v2, r) local c = math_cos(math_rad(r)); local s = math_sin(math_rad(r)); return new_vector2(c * v2.X - s*v2.Y, s*v2.X + c*v2.Y); end;
...
local library = {
    settings = {
        ...
        advanced_visible_check = false;
        visible_check_origin = "Head";
        friendly_color = Color3.new(0.576470, 0.254901, 1);
        flags = {};
    };
    overrides = {};
    cache = {};
...
do
    ...
    library.pass_through = function(from, target, raycast_params, ignore_table)
        ...
        if result then
            ...
            if is_descendant_of(instance, target.Parent)  then
                passed = true;
                return true;
            elseif library.settings.advanced_visible_check and instance.CanCollide == false or instance.Transparency == 1 then
    ...
    library.check_visible = function(target, from_head)
        ...
        --
        local from = from_head and localplayer.Character.Head.Position or camera.CFrame.p;
...
do
    ...
    library.get_health = function(plr)
        ...
        if character then
            ...
            if humanoid then
                return math_abs(math_floor(humanoid.Health)), math_abs(math_floor(humanoid.MaxHealth));
            end;
        end;
        return 100, 100;
    end;
        ...
        if plr ~= localplayer then
            ...
            data.connection = run_service.RenderStepped:Connect(function()
                ...
                if head and library.settings.enabled then 
                    on_screen = false;
                    if library.settings.visible_check_origin == "Head" then
                        is_visible = library.check_visible(head, true);
                    else
                    ...
                    if table_find(library.checks, "Team") and localplayer.Team == plr.Team then library.set_invisible(obj, true, true); return; end;
                    if table_find(library.checks, "Invisible") and head.Transparency == 1 then library.set_invisible(obj, true, true); return; end;
                    ...
                    if library.settings.max_distance > current_conversion then
                        if library.settings.box.type == "Dynamic" then 
                            ...
                            -- // Bounding Box
                            local box_size = library.round(new_vector2(math_max(width, 6), math_max(height, 10)));
                        ...
                        if library.settings.chams.enabled and library.settings.chams.type == "Highlight" then
                            ...
                            chams.FillColor = is_visible and library.settings.chams.color or library.settings.chams.invis_color;
        ...
        pcall(function()
            if library.cache[plr] then
                for i,v in next, library.cache[plr] do
                    if i ~= "chams" and i ~= "connection" then
                        destory_render_object(v);
